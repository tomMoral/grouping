...
\end{bmatrix}$$ avec $L = T-K$. Cette matrice contient toutes les sous séries de longueur $K$ de $f$. \textcolor{red}{Attention il faut utiliser l'environnement equation et pas les signes double dollar + Probleme de notation : X est une matrice L par K non ? Bien vérifier}
...
$$où  $U_i, V_i$ sont les colonnes des matrices $U, V$. Grâce à une opération d'hankelisation, on peut transformer chaque matrice de trajectoire $\lambda_iU_iV_i^T$  en un signal temporel $f^{(i)}$, et une décomposition de $f$ sous la forme 
$$f = \sum_{i=1}^K f^{(i)}$$ où $f^{(i)} $ est la composante associée avec le triplet propre $(\lambda_i, U_i, V_i)$. On appellera $U_i$ le motif associé et $\lambda_i$ la valeur singulière associée.
...
$$où $Z$ est une constante de normalisation telle que le périodogramme ait une norme unitaire. Les composantes contenant une même phase oscillante partagent des structures communes dans leur périodogramme qui peuvent permettre leur identification.
...
\paragraph{Similarité des pics (GG2)}\label{par:GG2}
    ...
    g_{i, j} = \begin{cases}
	    ...
	    0 & \text{ sinon}
    \end{cases}
    $$ où $m = \min\left(\left|F_{f^{(i)}}\right|, \left|F_{f^{(j)}}\right|\right)$. Cette métrique tente de rendre plus robuste la détection de composantes ayant le même périodogramme en comparant non plus le pic maximal (HG) mais les supports spectraux des composantes. 
% paragraph GG2 (end)
...
On calcule alors le rappel et la précision du regroupement pour $R^2$.
    ...
    $$ \textcolor{red}{Decris tes notations !, permutations etc etc}
