...
% TODO - Attention il faut utiliser l'environnement equation et pas les signes double dollar
% TODO - Bien véridier les notations
...
% TODO - Qu'est ce que l'hankelisation ???
...
\paragraph{Méthode hiérarchique (MH)}
    ...
    %TODO - vérifier que les notations sont claires et bien définies
    On peut donc considérer qu'on ne rajoute une composante $j$ dans un groupe $I_k$ que si elle est similaire à la composante dominante du groupe \emph{ i.e.} $a_{i, j} = 1$ pour $i = \arg\max_{l \in I_k} \lambda_l$.
...
\label{sub:met}
    ...
    \end{equation*}
    où $\mathcal S(M)$ est le groupe des permutations de $\set{m}$ et 
    %TODO - Décrire les notations
...
%TODO - Il faut aller plus loin dans l'analyse !
